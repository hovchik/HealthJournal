...
import com.hovchik.healthjournal.domain.model.ai.CompatibilityReport
import com.hovchik.healthjournal.domain.model.ai.DeviceCapabilityResult
import com.hovchik.healthjournal.domain.model.ai.LocalAiModel
...
class ModelCompatibilityValidator(
    ...
    fun validate(model: LocalAiModel, capability: DeviceCapabilityResult? = null): CompatibilityReport {
        ...
        // ABI check
        if (model.runtimeType == "llama_cpp") {
        ...
        // SDK check
        if (model.runtimeType == "llama_cpp" && Build.VERSION.SDK_INT < 24) {
            issues.add("llama.cpp requires Android API 24+")
        }
